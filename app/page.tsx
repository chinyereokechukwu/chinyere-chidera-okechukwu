...
export default function Home() {
  ...
  const featuredInitiatives = initiatives.filter((initiative) => initiative.id === "impactbridge" || initiative.id === "wordrootedstudio");
  return <>
    ...
    <section className="section"><div className="container"><div className="eyebrow">Initiatives</div><div className="initiative-grid">{featuredInitiatives.map((initiative) => <article className="initiative-card" key={initiative.id}><div><h2 className="serif">{initiative.title}</h2><p>{initiative.subtitle}</p></div><Link href={initiative.id === "impactbridge" ? "/impactbridge" : "/wordrootedstudio"} className="button button-primary">Learn More <ArrowRight size={18}/></Link></article>)}</div></div></section>
